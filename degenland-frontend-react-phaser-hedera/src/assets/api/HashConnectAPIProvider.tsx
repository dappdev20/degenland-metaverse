...
export default function HashConnectProvider({
  ...
  const sendHbarToTreasury = async (amount_) => {
    ...
    console.log("************************ sendHbarAndNftToTreasury 1 : ", allowanceRx);
  ...
  const receiveMultipleNfts = async (nftInfo_) => {
    const _accountId = saveData.accountIds[0];
    const _provider = hashConnect.getProvider(netWork, saveData.topic, _accountId);
    const _signer = hashConnect.getSigner(_provider);
  ...
  return (
    <HashConnectAPIContext.Provider
      value={{ walletData: saveData, installedExtensions, connect, disconnect, sendHbarAndNftToTreasury, sendHbarToTreasury, receiveNft, sendHbarAndMultiNftsToTreasury, receiveMultipleNfts }}>
      {children}
    </HashConnectAPIContext.Provider>
  );
}
